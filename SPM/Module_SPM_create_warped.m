function [files_in,files_out,opt] = Module_SPM_create_warped(files_in,files_out,opt)
...
if isempty(opt)
    ...
    module_option(:,3)   = {'Modulation','Pres. Concentration (No "modulation")'};
    module_option(:,4)   = {'Time_Steps','64'};
    module_option(:,5)   = {'Interpolation','Trilinear'};
    ...
    user_parameter(:,1)   = {'Description','Text','','','', '',...
       {'This allows spatially normalised images to be generated. Note that voxel sizes and bounding boxes can not be adjusted,'
       'and that there may be strange effects due to the boundary conditions used by the warping.'
       'Also note that the warped images are not in Talairach or MNI space.'
       'The coordinate system is that of the average shape and size of the subjects to which Shoot was applied.'
       'In order to have MNI-space normalised images, then the Deformations Utility can be used to compose the individual Shoot warps,'
       'with a deformation field that matches (e.g.) the Template grey matter generated by Shoot, with one of the grey matter volumes released with SPM.'}}  ;
    ...
    user_parameter(:,5)   = {'   .Modulation','cell',{'Pres. Concentration (No "modulation")','Pres. Amount ("Modulation")'},'Modulation','','',...
       {'This allows the spatially normalised images to be rescaled by the Jacobian determinants of the deformations.'
       'Note that the rescaling is only approximate for deformations generated using smaller numbers of time steps.'
       ''
       'One of the following options must be selected'
       '   Pres. Concentration (No "modulation")'
       '   Pres. Amount ("Modulation")'
       }'};
    user_parameter(:,6)   = {'   .Time Steps','cell',{'1', '2','4','8','16','32','64','128','256','512'},'Time_Steps','','',...
...
if isempty(files_out)
    ...
    for i=1:length(files_in.In1)
        ...
        table_out_tmp.SequenceName = categorical(cellstr(['w', char(table_out_tmp.SequenceName)]));
...
if ~exist('files_in','var')||~exist('files_out','var')||~exist('opt','var')
    error('Module_SPM_create_warped:brick','Bad syntax, type ''help %s'' for more info.',mfilename)
end
...
switch opt.Interpolation
    ...
    case '2nd Degree B-Spline'
        matlabbatch{1}.spm.tools.dartel.crt_warped.interp = 2;
    case '3rd Degree B-Spline'
        matlabbatch{1}.spm.tools.dartel.crt_warped.interp = 3;
    ...
    case '6th Degree B-Spline'
        matlabbatch{1}.spm.tools.dartel.crt_warped.interp = 6;
    case '7th Degree B-Spline'
        matlabbatch{1}.spm.tools.dartel.crt_warped.interp = 7;
end

switch opt.Time_Steps
    case  '1'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 0;
    case   '2'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 1;
    case   '4'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 2;
    case   '8'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 3;
    case    '16'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 4;
    case   '32'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 5;
    case   '64'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 6;
    case    '128'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 7;
    case    '256'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 8;
    case    '512'
        matlabbatch{1}.spm.tools.dartel.crt_warped.K = 9;
end
...
matlabbatch{1}.spm.tools.dartel.crt_warped.flowfields =  {tmp_flowfield_fullfilename};
...
for i=1:length(files_in.In1)
    if ~isempty(files_in.In1{i})
        ...
        if numel(header) > 1
            ...
            images_list{size(images_list,2)+1} = {tmp_images_fullfilename};
        end
    end
end
matlabbatch{1}.spm.tools.dartel.crt_warped.images = images_list;

switch opt.Modulation
    case 'Pres. Concentration (No "modulation")'
        matlabbatch{1}.spm.tools.dartel.crt_warped.jactransf = 0;
    case 'Pres. Amount ("Modulation")'
...
for i=1:length(files_in.In1)
    ...
    movefile(strcat(opt.Table_in.Properties.UserData.MP3_data_path, 'Tmp', filesep, ['w', char(opt.Table_in.Filename(i))],'.nii'), output_fullfilename);
    if exist(strrep(files_in.In1{i},'.nii','.json'), 'file')
