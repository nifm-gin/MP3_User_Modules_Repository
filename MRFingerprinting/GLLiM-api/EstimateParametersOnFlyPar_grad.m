function [Yestim, score, Xestim] = EstimateParametersOnFlyPar_grad(Xobs, Xgrid, Ygrid, geo, Ylabels)
...
gradindex = find(strcmp('gradZ', Ylabels), 1);
...
ps.SchedulerComponents(1).NumWorkers = 42;
ps.SchedulerComponents(1).NumThreads = 3;
...
for j = 1:numel(rI)
    ...
    Yestim(rI(j), gradindex) = geo.(fn{distribIdx(cI(j))}).grad;
end
