function [files_in,files_out,opt] = Module_MGEFIDSE_conc2(files_in,files_out,opt)
...
if isempty(opt)
    ...
    user_parameter(:,6)   = {'   .Parameters','check', ...
        {'Vf', 'VSI', 'R', 'SO2', 'DH2O', 'B0theta', 'khi', 'Hct', 'T2', 'B0off', 'Label', 'T1', 'df', 'gamma', 'B1rel', 'Ttwo_star'},...
        'Params', '', '',...
...
if opt.removed > 0
    Xpre = Xpre(:,:,:, 1:end-opt.removed);
%     for x = 1:size(Xpre,1)
%         for y = 1:size(Xpre,2)
%             for z = 1:size(Xpre,3)
%                 tmp(x,y,z,:) = Xpre(x,y,z,1:end-opt.removed);
%             end
%         end
%     end
%     Xpre    = tmp;
%     clear tmp
    Xpost = Xpost(:,:,:, 1:end-opt.removed);
%     for x = 1:size(Xpost,1)
%         for y = 1:size(Xpost,2)
%             for z = 1:size(Xpost,3)
%                 tmp(x,y,z,:) = Xpost(x,y,z,1:end-opt.removed);
%             end
%         end
%     end
%     Xpost= tmp;
...
%     if size(Xobs,length(size(Xobs)))/2 ~= size(Dico.MRSignals{1},2) % Divided by two because Xobs = [Xpre Xpost]
    if size(Xobs, 3) ~= (size(Dico.MRSignals{1}, 2) + size(Dico.MRSignals{2}, 2))
