function [files_in,files_out,opt] = Module_Slice2Echo(files_in,files_out,opt)
...
%     % --> module_option(2,:) = defaults values    
    ...
    module_option(:,3)   = {'output_filename_ext','_S2E'};
...
% 
    ...
    user_parameter(:,1)   = {'Description','Text','','','', '', {'Reshapes a X*Y*N scan to X*Y*SliceNumber*Echos, for MRF dicoms wrongly imported '}};
...
[X, Y, nImages] = size(N);
nEchoes = nImages / opt.SliceNumber;
NewIm = reshape(N, [X, Y, nEchoes, opt.SliceNumber]);
...
if numel(info.PixelDimensions) == 3
    ...
    info2.PixelDimensions = info.PixelDimensions;
end
