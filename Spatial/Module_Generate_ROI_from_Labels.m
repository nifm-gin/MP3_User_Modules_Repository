function [files_in,files_out,opt] = Module_Generate_ROI_from_Labels(files_in,files_out,opt)
...
if isempty(opt)
    ...
    module_parameters(:,1)   = {'output_filename','Name_of_the_ROI'};
    module_parameters(:,2)   = {'OutputSequenceName','AllName'};
    module_parameters(:,3)   = {'Labels_to_keeps',''};
    ...
    user_parameter(:,1)   = {'Description','Text','','','','',...
        {
        'This module create a ROI from a label scan.'
        'User have to select the label(s) he/she wants to convert to an ROI scan'
        }'};
    user_parameter(:,2)   = {'Select one Atlas (or cluster) scan as input','1Cluster','','',{'SequenceName'}, 'Mandatory',''};
    user_parameter(:,3)   = {'Parameters','','','','', '', ''};
    user_parameter(:,4)   = {'   .Output filename','char','','output_filename','', '','The use have to enter the name of the ROI that will be generated'};
    user_parameter(:,5)   = {'   .Select Label(s) to convert to an ROI :','numeric','','Labels_to_keeps','', '',...
        {
...
if ~exist('files_in','var')||~exist('files_out','var')||~exist('opt','var')
    error('Module_Generate_ROI_from_Labels:module','Bad syntax, type ''help %s'' for more info.',mfilename)
end
